...
import grade.PassFailEvaluation; //추가
...
public class GenerateGradeReport {
	...
	public void makeBody(Subject subject){
		...
		for(int i=0; i<studentList.size(); i++){
			...
			getScoreGrade(student, subject);  //학생별 해당과목 학점 계산   subjectId -> subject
			buffer.append("\n");
			buffer.append(LINE);
		}
	}	
	
	// 핵심 학점 구하는 메서드  - makeBody메서드의 일부
	// 추가: int subjectId -> Subject subject
	public void getScoreGrade(Student student, Subject subject){
		...
		GradeEvaluation[] gradeEvaluation = {new BasicEvaluation(), new MajorEvaluation(), new PassFailEvaluation()}; //추가 : 학점 평가 클래스들
